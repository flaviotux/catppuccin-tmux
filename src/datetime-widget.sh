...
# Check if enabled
ENABLED=$(tmux show-option -gv @catppuccin_show_datetime 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
date_format=$(tmux show-option -gv @catppuccin_date_format 2>/dev/null)
time_format=$(tmux show-option -gv @catppuccin_time_format 2>/dev/null)
...
echo "$RESET#[fg=${THEME[text]},bg=${THEME[surface0]}] $date_string $separator$time_string "
