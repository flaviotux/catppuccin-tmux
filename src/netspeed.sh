...
# Check if enabled
ENABLED=$(tmux show-option -gv @catppuccin_show_netspeed 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
INTERFACE=$(tmux show-option -gv @catppuccin_netspeed_iface 2>/dev/null)
# Show IP address
SHOW_IP=$(tmux show-option -gv @catppuccin_netspeed_showip 2>/dev/null)
# Time between refresh
TIME_DIFF=$(tmux show-option -gv @catppuccin_netspeed_refresh 2>/dev/null)
...
NET_ICONS[traffic_tx]="#[fg=${THEME[sapphire]}]\U000f06f6"    # nf-md-upload_network
NET_ICONS[traffic_rx]="#[fg=${THEME[teal]}]\U000f06f4"   # nf-md-download_network
...
if [[ -z $INTERFACE ]]; then
  ...
  tmux set-option -g @catppuccin_netspeed_iface "$INTERFACE"
fi
