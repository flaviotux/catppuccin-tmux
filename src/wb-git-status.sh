...
ENABLED=$(tmux show-option -gv @catppuccin_show_wbg)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
if [[ $PR_COUNT -gt 0 ]]; then
  PR_STATUS="#[fg=${THEME[green]},bg=${THEME[mantle]},bold] ${RESET}${PR_COUNT} "
fi

if [[ $REVIEW_COUNT -gt 0 ]]; then
  REVIEW_STATUS="#[fg=${THEME[yellow]},bg=${THEME[mantle]},bold] ${RESET}${REVIEW_COUNT} "
fi

if [[ $ISSUE_COUNT -gt 0 ]]; then
  ISSUE_STATUS="#[fg=${THEME[green]},bg=${THEME[mantle]},bold] ${RESET}${ISSUE_COUNT} "
fi

if [[ $BUG_COUNT -gt 0 ]]; then
  BUG_STATUS="#[fg=${THEME[red]},bg=${THEME[mantle]},bold] ${RESET}${BUG_COUNT} "
fi

WB_STATUS="#[fg=${THEME[surface1]},bg=${THEME[mantle]},bold] $RESET$PROVIDER_ICON $RESET$PR_STATUS$REVIEW_STATUS$ISSUE_STATUS$BUG_STATUS"
