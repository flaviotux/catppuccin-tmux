...
tmux set -g message-style "bg=${THEME[blue]},fg=${THEME[surface0]}"
tmux set -g message-command-style "fg=${THEME[blue]},bg=${THEME[surface0]}"
...
zoom_id_style="$(echo "$TMUX_VARS" | grep '@catppuccin_zoom_id_style' | cut -d" " -f2)"
terminal_icon="$(echo "$TMUX_VARS" | grep '@catppuccin_terminal_icon' | cut -d" " -f2)"
active_terminal_icon="$(echo "$TMUX_VARS" | grep '@catppuccin_active_terminal_icon' | cut -d" " -f2)"
window_tidy="$(echo "$TMUX_VARS" | grep '@catppuccin_window_tidy_icons' | cut -d" " -f2)"
...
#+--- Bars LEFT ---+
# Session name
tmux set -g status-left "#[fg=${THEME[mantle]},bg=${THEME[blue]},bold] #{?client_prefix,󰠠 ,#[dim]󰤂 }#[bold,nodim]#S$hostname "
